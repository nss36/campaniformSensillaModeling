...
%   Detailed explanation goes here
    ...
    if m > 0 && size(A,1) < size(A,2) && size(A,1) == m && rank(A) == m && size(A,2) == n
...
%         if all(A*x0-b == zeros(m,1))
        ...
        elseif size(A,2) ~= n
            error('Equality constraints are not configured correctly. A and b matrices are likely the wrong size.')
        end
        nonbasic_to_full_map = @(x)x;
        x0_nonbasic = x0;
        nonbasic_var = (1:n)';
