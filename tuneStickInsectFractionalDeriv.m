...
for i=1:8
    ...
    meanAbsErr6A(i) = mean(abs(cleanedData{i}.sens6A - sim6A));
    
    ylabel('6A discharge')
    legend('animal',sprintf('model, err = %3.1f',meanAbsErr6A(i)))
    ylim([0,300])
    ...
    meanAbsErr6B(i) = mean(abs(cleanedData{i}.sens6B - sim6B));
    
    ylabel('6B discharge')
    legend('animal',sprintf('model, err = %3.1f',meanAbsErr6B(i)))
    xlabel('time (s)')
    ylim([0,300])
